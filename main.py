...
# Ensure sample sizes do not exceed balanced dataset size
sizes = [min(s, N) for s in sizes]

print(f"Adjusted sample sizes: {sizes}")

# 3. Create five samples from balanced data -----------------------------------
samples = [
    balanced.sample(n=sizes[i], random_state=42 + i).reset_index(drop=True)
    for i in range(5)
]

# 4. Define samplers & models -------------------------------------------------
samplers = {
...
# 6. Average accuracies over five samples -------------------------------------
